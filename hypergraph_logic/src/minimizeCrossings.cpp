...
namespace sifting_internal {
	...
	std::vector<int> collectChainG1Nodes(int start_g1, const SiftState& S, std::unordered_set<int>& visited) {
		...
		auto children = S.g1_nodes[start_g1].original->getChildren();
		Node* current = children[0].get(); // We already know this is the only child, so we can directly access it.
		...
		int layer)
	{
		...
		// Sort based on lexicographical order of minimum parent position and minimum child position, as described in header file.
		...
		const std::vector<std::vector<int>>& connections_out)
	{
		...
		int p = static_cast<int>(layer1.size()); // northern layer size
		int q = static_cast<int>(layer2.size()); // southern layer size
		pos1.reserve(p);
		pos2.reserve(q);
		...
		// --- Step 1: Build the edge sequence π ---
		// Sort edges lexicographically by (north_pos, south_pos),
		// then extract just the south positions to construct sequence π.
		...
		// π is the sequence of southern positions in sorted edge order
		std::vector<int> pi;
		pi.reserve(edges.size());
		for (auto& [n, s] : edges)
			pi.push_back(s);

		// --- Step 2: Count inversions via accumulator tree (BJM) ---
		// Build a perfectly balanced binary tree with 2^c leaves, where
		// 2^(c-1) < q <= 2^c. Leaves 0..q-1 correspond to south positions.
		int firstindex = 1;
		while (firstindex < q) firstindex *= 2;
		int treesize = 2 * firstindex - 1;
		int leafOffset = firstindex - 1; // index of leftmost leaf in the array

		std::vector<int> tree(treesize, 0);

		int crosscount = 0;
		for (int southPos : pi) {
			int index = southPos + leafOffset; // start at the corresponding leaf
			tree[index]++;
			while (index > 0) {
	...
	static void orderLayersByBlockOrder(SiftState& S, const BlockList& B) {
		...
		for (auto& [layer, nodes] : S.g1_layers) {
			std::sort(nodes.begin(), nodes.end(),
				[&](int idx_a, int idx_b) {
					int block_a = S.g1_nodes[idx_a].block_id;
					int block_b = S.g1_nodes[idx_b].block_id;
					return S.pi[block_a] < S.pi[block_b];
				});
		}
	}
